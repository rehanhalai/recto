      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {users.map((user: any, i: number) => (
          <Link
            href={`/${user.userName}`}
            key={`${user.id}-${i}`}
            className="group flex items-center gap-4 p-4 rounded-xl bg-card-surface border border-border-subtle hover:border-accent/50 transition-all hover:shadow-md"
          >
            <UserAvatar
              src={user.avatarImage || null}
              fallbackName={user.fullName || user.userName}
              className="h-14 w-14 ring-2 ring-transparent transition-all group-hover:ring-accent/30"
            />
